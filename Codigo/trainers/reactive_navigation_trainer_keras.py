...
@baseline_registry.register_trainer(name="reactive_keras")
class ReactiveNavigationTrainerKeras(BaseRLTrainer):
    """
    ...
    This trainer is deprecated due to problems with Tensorflow, please use ReactiveNavigationTrainer instead
    """
    ...
    def _train_network_standard(self):
        ...
        current_state_predictions = self._q_network.predict(sampled_initial_states, self._training_batch_size)
        ...
        self._q_network.fit_model(sampled_initial_states, current_state_predictions, self._training_batch_size)
    ...
    def _clean_up(self):
        """
        As time goes on, Keras and Tensorflow usage of memory increases,
        which can lead to a crash eventually

        To avoid this, this method:
         * cleans the General Session stored by Keras in order to free memory
         * wipes the Garbage Collector (if needed) to avoid memory leaks
        """

        # Clean the Keras backend
        tf.keras.backend.clear_session()

        # Every 50 episodes, forces the Garbage Collector to act
        if self.num_updates_done % 50 == 0:
            gc.collect()
    ...
    def train(self):
        ...
        while not self.is_done():
            ...
            # Clean up to reduce GPU and memory usage
            self._clean_up()
